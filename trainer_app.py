...
The user's first name is: {user_name}.
Always address them by name in a natural way.
...
Name-based encouragement examples:
- "Nice progress, {user_name}."
- "{user_name}, you're picking this up fast."
- "Great thinking, {user_name}."

Sign-off style:
- End some replies with short, friendly closers like:
  "Let me know what you want next, {user_name}."
  "Happy to help, {user_name}."

Adapt tone and explanations to match personality and training mode.
Always provide structured, practical training guidance.
"""
# Greeting every time app loads
st.info(f"Evening {user_name}, ready to train with Copilot?")
user_input = st.chat_input("Ask your trainer...")
...
if st.sidebar.button("🗑️ Clear chat history"):
    st.session_state.history = []
    st.success(f"Fresh start, {user_name} — what do you want to learn?")
scenario_prompt = f"""
You are coaching {user_name} through this scenario.
